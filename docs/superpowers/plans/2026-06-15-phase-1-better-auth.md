...
| Q3 | internalAdapter shapes (T8) | ⚠️ **Residual** — internal API, not in docs. Resolved by the **Task 4 spike** against `node_modules/better-auth` types. Fallback: public `auth.api.setUserPassword` for the credential-creation step. |
...
**Two residual items carry into execution** (both have a defined resolution mechanism, not blockers):
1. **Q3** — exact `internalAdapter` method/arg shapes → confirmed by the Task 4 spike before any code depends on them.
2. **Q9** — the seed's transitive `next/headers` import → verified at Task 15 Step 2, with a one-file refactor as the fallback.
